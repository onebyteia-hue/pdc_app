...
async function callAI({ prompt, feature }) {
  if (!currentUser)
  ...
  const r = await fetch(GEMINI_ENDPOINT, {
    ...
    body: JSON.stringify({ prompt, feature, idToken: token }),
  });

  if (!r.ok) {
    const t = await r.text().catch(() => "");
    throw new Error(t || `Error del servidor: ${r.status}`);
  }
  return await r.json();
}
...
// async function callAI({ prompt, feature }) {
//   if (!currentUser) throw new Error("Sesión inválida. Vuelve a iniciar sesión.");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    if (window.__PDC_CURRENT_PROFILE__) {
...
const generarObjetivoConIA = async () => {
  ...
  //const promptTexto = `Actúa tu como pedagogo boliviano. Redacta un OBJETIVO HOLÍSTICO para ${nivelText}, ${escolaridadText}, área ${areaText}. Temas: ${listaContenidos}. Referencia: ${objNivelRef}. Formato: Un párrafo, dimensiones Ser, Saber, Hacer, Decidir. Times New Roman 9, sin negritas, sin asteriscos.`;
...
Entrega solo el objetivo final, sin títulos ni explicaciones.`;
  ...
  try {
    ...
    const out = (data?.texto ?? "").trim();
    campoDestino.innerText = out ? out : "(sin texto devuelto)";
...
async function generarAdaptacionesEspecificasConIA() {
  ...
  const contenido =
    document.getElementById("adapt-esp-contenido")?.value?.trim() || "";
  const condicion =
    document.getElementById("adapt-esp-condicion")?.value?.trim() || "";

  const outAda = document.getElementById("p-adapt-esp-adaptacion");
  const outSer = document.getElementById("p-adapt-esp-ser");
  const outSaber = document.getElementById("p-adapt-esp-saber");
  const outHacer = document.getElementById("p-adapt-esp-hacer");

  if (!outAda || !outSer || !outSaber || !outHacer) return;

  if (!contenido || !condicion) {
    alert(
      "Completa el Contenido a desarrollar y la Condición (discapacidad/talento/dificultad).",
    );
    return;
  }
  ...
  try {
    setTyping(outAda, "✨ Generando adaptación");
    setTyping(outSer, "✨ Generando SER");
    setTyping(outSaber, "✨ Generando SABER");
    setTyping(outHacer, "✨ Generando HACER");
...
4) Viñetas breves, observables y evaluables (máx 10 palabras).`;
    ...
    outAda.innerText = "Error al generar adaptaciones específicas.";
...
async function generarMomentosConIA() {
  ...
  try {
    for (const index of indices) {
      ...
      const prompt = `Actúa como docente experto en Planificación de Desarrollo Curricular en Bolivia.
...
2) En recursos, usa viñetas (arreglo) de recursos concretos (máx 10).
3) Ajusta a nivel escolar (lenguaje sencillo y viable en aula).
4) Parrafos de max. 12 palabras.`;
      ...
      try {
        ...
        // fallback: intentar extraer el primer bloque JSON si viene envuelto
        const m = String(texto).match(/\{[\s\S]*\}/);
        if (m) {
          try {
            parsed = JSON.parse(m[0]);
